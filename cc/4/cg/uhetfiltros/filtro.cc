...
Matrix* Filtro::threshold(){
	return this->threshold(127);
}

Matrix* Filtro::threshold(int limite){
...
int Filtro::calcDistancia(int pa, int pb){

	return (int) rint(sqrt( ((pa-pb)*(pa-pb)) *3)) & 0xFF;
}

Matrix* Filtro::deteccao_de_borda(){
	
	return this->deteccao_de_borda(50);
}

Matrix* Filtro::deteccao_de_borda(int distancia){
...
Matrix* Filtro::extracao_de_cor(){
	
	return this->extracao_de_cor(50, BRANCO);
}

Matrix* Filtro::extracao_de_cor(int distancia, int cor){
...
Matrix* Filtro::mediana(){

	return this->mediana(3);
}

int get_pos_menor(int* v, int ini, const int tam){
...
void selection(int* v, const int tam){
...
fMatrix* Filtro::calcJanela(int dimensao, float borda, float centro){
...
int Filtro::calcCor(int i2, int j2, fMatrix* mfiltro, float divisor){
	...
	for (int i = 0 ; i < mfiltro->getLinhas(); i++){
		for (int j = 0 ; j < mfiltro->getColunas(); j++){
			acm += mfiltro->get(i,j) * this->M->get( i+i2-recuo , j+j2-recuo );
		}			
	}
	return (int) rint(acm / divisor);
}

Matrix* Filtro::passa_alta(){
	return this->passa_alta(3);
}

Matrix* Filtro::passa_alta(int n){

	Matrix* maux = this->M->cpy();
	fMatrix* mfiltro = this->calcJanela(n,-1,n*n);
	int recuo = (int) (n-1)/2;
...
Matrix* Filtro::passa_baixa(){

	return this->passa_baixa(3);
}

Matrix* Filtro::passa_baixa(int dimensao){
...
Matrix* Filtro::roberts(){

	Matrix* maux = this->M->cpy();
	fMatrix* mV = new fMatrix(2,2);
	fMatrix* mH = new fMatrix(2,2);

	mV->set(0,0, 0); mV->set(0,1, 1);
	mV->set(1,0,-1); mV->set(1,1, 0);

	mH->set(0,0, 1); mH->set(0,1, 0);
	mH->set(1,0, 0); mH->set(1,1,-1);
...
Matrix* Filtro::sobel(){
	...
	mV->set(0,0,-1); mV->set(0,1, 0); mV->set(0,2, 1);
	mV->set(1,0,-2); mV->set(1,1, 0); mV->set(1,2, 2);
	mV->set(2,0,-1); mV->set(2,1, 0); mV->set(2,2, 1);

	mH->set(0,0, 1); mH->set(0,1, 2); mH->set(0,2, 1);
	mH->set(1,0, 0); mH->set(1,1, 0); mH->set(1,2, 0);
	mH->set(2,0,-1); mH->set(2,1,-2); mH->set(2,2,-1);
