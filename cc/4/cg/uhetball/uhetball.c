...
//					if (g_jogo->n_blocos <= 0) glutLeaveMainLoop();
					system("play --silent ./sound/exp.wav 2> /dev/null &");
//					uhet_show_msg("opa!\n");
...
void uhet_show_game_help(){
	...
	glPushMatrix();
		...
		glPushMatrix();
			...
			uhet_show_game(GLUT_BITMAP_8_BY_13, "* a,d,w,s - controla o zoom\0");
		glPopMatrix();
	glPopMatrix();
...
static void uhet_movimento_colisao(){
	...
	if (pos_y_bola < uhet_obj_box_get(g_jogador,Y_INI)){
		...
		if ((pos_x_bola >= pos_xi_jogador) && ((pos_x_bola <= pos_xf_jogador))){
				...
				system("play --silent ./sound/chaos.wav 2> /dev/null &");
			...
			system("play --silent ./sound/damn.wav 2> /dev/null &");
		}
